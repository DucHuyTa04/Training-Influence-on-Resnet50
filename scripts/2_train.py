...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device,
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in pbar:
                ...
                # Step scheduler per batch if required (e.g., OneCycleLR)
                if phase == 'train' and scheduler_step_per_batch:
                    scheduler.step()
            ...
            if phase == 'train':
                ...
                if not scheduler_step_per_batch:
                    if hasattr(scheduler, 'step') and 'metrics' in scheduler.step.__code__.co_varnames:
                        ...
                        # Other schedulers (CosineAnnealing, etc.) don't need loss
                        scheduler.step()
    ...
    if save_model:
        ...
        model_save_path = os.path.join("models/best", f"Resnet50_animals10_val_{acc_str}_{loss_str}.pth")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-head_epochs", type=int, default=5, dest="head_epochs", help="Number of epochs for head-only training")
    parser.add_argument("-batch_size", type=int, default=64, dest="batch_size", help="Batch size")
    parser.add_argument("-lr", type=float, default=1e-4, dest="lr", help="Learning rate for fine-tuning")
    parser.add_argument("-early_stopping_patience", type=int, default=15, dest="early_stopping_patience", help="Early stopping patience")
    ...
    parser.add_argument("-checkpoint_freq", type=int, default=10, dest="checkpoint_freq", help="Save checkpoint every N epochs")
    ...
    full_train_dataset = datasets.ImageFolder(root=str(data_dir / 'train'), transform=data_transforms['train'])
    ...
    optimizer_ft = optim.AdamW([
        {'params': backbone_params, 'lr': lr * 0.1},  # 10x lower LR for pretrained backbone
        {'params': head_params, 'lr': lr * 2}         # 2x base LR for head (already trained in stage 1)
    ], weight_decay=1e-4)

    # Use ReduceLROnPlateau - reduces LR when validation loss plateaus
    ...
    model, history = train_model(model, criterion, optimizer_ft, scheduler_ft,
                                 ...
                                 checkpoint_dir=str(script_dir / 'models' / 'checkpoints'),
                                 stage_name='finetune',
                                 scheduler_step_per_batch=False)  # ReduceLROnPlateau steps per epoch based on val loss
