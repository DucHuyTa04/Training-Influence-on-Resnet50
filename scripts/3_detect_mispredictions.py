...
Outputs:
 - outputs/mispredictions/false_predictions.csv (columns: image_path,split,true_class_index,pred_class_index)
 - outputs/mispredictions/mispredictions_grid.png (visual grid)
...
MODEL_PATH = "models/best/Resnet50_animals10_val_0_9813_0_6168.pth"
...
# Attempt to map each mispredicted image to the original image under data/raw (match by filename)
# The raw images live at <script_dir>/data/raw (not under data/processed)
...
if os.path.isdir(raw_dir):
    ...
    print(f"Warning: raw directory not found at {raw_dir}. Keeping processed image paths.")

# Replace the stored path with the raw path when a match is found (match by basename).
not_found = 0
ambiguous = 0
...
if raw_map:
    ...
    print(f"Raw matches: {matched}, not found in raw: {not_found}, ambiguous matches: {ambiguous}")

# Save CSV (paths will point to raw-img when a match was found)
os.makedirs(script_dir, exist_ok=True)
csv_path = os.path.join(script_dir, 'outputs/mispredictions/false_predictions.csv')
...
out_dir = script_dir / 'outputs' / 'mispredictions'
...
combined_path = out_dir / 'mispredictions_grid.png'
if len(mis_all) == 0:
